...
from facial.facial_landmark import FaceMeshDetector

# pose estimation and stablization
from facial.pose_estimator import PoseEstimator
from facial.stabilizer import Stabilizer

# Miscellaneous detections (eyes/ mouth...)
from facial.facial_features import FacialFeatures, Eyes

import sys

# global variable
port = 8888         # have to be same as unity


## for expression estimation.
import inference
...
def main():
    ...
    while cap.isOpened():
        ...
        # first two steps
        img_facemesh, faces, emotion_tmp = detector.findFaceMesh(img, img_size=img_size)
        
        ## for emotion
        if len(emotion_tmp): # not empty
            query_face_mesh.append(emotion_tmp - emotion_tmp[0])

        if len(query_face_mesh) == 27:
            pred = inference.pred_emotion(emotion_cls_model, np.array(query_face_mesh))
            print(emotion_name[int(pred)])
