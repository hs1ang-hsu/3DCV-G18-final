...
def demo(opt, meta):
    ...
    if opt.demo == 'webcam' or \
        ...
        while (cam.isOpened()):
            ...
            #
            box = ret['boxes']
            print(type(box), len(box)) #list, len=# of detected objects
            for i in range(0, len(box)):
                #print(len(box[i])) #=5: projected_points, point_3d_cam, np.array(bbox['obj_scale']), points_ori, bbox
                #for i in range(0, len(box[0])):
                    #print(i, box[0][i])
                #sys.exit()
                
                t = box[i][4]['location']
                quat = box[i][4]['quaternion_xyzw']
            #
            idx = idx + 1
